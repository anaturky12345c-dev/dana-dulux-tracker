...
function renderDashboard(){
  ...
  const today=todayRiyadh(), followDue=state.reports.filter(r=>r.next_followup_date&&r.next_followup_date<=today).sort((a,b)=>a.next_followup_date.localeCompare(b.next_followup_date)); const followHtml=followDue.slice(0,8).map(r=>{const c=state.customers.find(x=>Number(x.id)===Number(r.customer_id)),link=waLink(c?.phone,`السلام عليكم، معك ${r.rep?.full_name||'مندوب دانة التاج'} من دانة التاج. نتابع معكم بخصوص ${r.next_action||'طلبكم'}.`);return `<div class="event ${r.next_followup_date<today?'overdue-event':''}"><b>${esc(r.customer?.name||'-')}</b><div>${r.next_followup_date<today?'متابعة متأخرة':'متابعة اليوم'} — ${dateOnly(r.next_followup_date)}</div><div class="small">${esc(r.next_action||r.note)}</div>${link?'<a class="btn good wa-btn" target="_blank" rel="noopener" href="'+link+'">واتساب العميل</a>':''}</div>`}).join('');
  const attention=state.reports.filter(r=>r.report_type==='management'); $('attentionList').innerHTML=(followHtml?'<h4>متابعات اليوم والمتأخرة</h4>'+followHtml:'')+(attention.length?'<h4>تدخل الإدارة</h4>'+attention.slice(0,8).map(r=>`<div class="event"><b>${esc(r.customer?.name||'-')}</b><div>${esc(r.note)}</div><div class="small">${dateTime(r.created_at)} — ${esc(r.rep?.full_name||'-')} — ${esc(r.next_action||'')}</div></div>`).join(''):'')||'<div class="small">لا توجد حالات حالياً.</div>';
  if(isAdmin()){
...
function waLink(phone,text){const p=String(phone||'').replace(/\D/g,'').replace(/^0/,'966');return p?'https://wa.me/'+p+'?text='+encodeURIComponent(text):'';}
