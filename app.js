...
async function renderMap(){
  if(!isAdmin())return;
  if(!state.map){state.map=L.map('map').setView([24.78,46.76],11);L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20,attribution:'© OpenStreetMap contributors © CARTO'}).addTo(state.map);state.markerLayer=L.layerGroup().addTo(state.map);}
