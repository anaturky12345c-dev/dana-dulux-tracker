...
function updateInitialStatusReason(){
 const status=$('fStatus')?.value,wrap=$('fInitialReasonWrap'),label=$('fInitialReasonLabel');if(!wrap)return;
...
async function createCustomer(){
 const name=$('fName').value.trim(),area=$('fArea').value.trim(),phone=$('fPhone').value.trim(),status=$('fStatus').value,lat=$('fLat').value,lng=$('fLng').value,reason=$('fInitialReason')?.value.trim()||'';
 ...
 const {error}=await sb.rpc('create_customer',{p_name:name,p_area:area||null,p_phone:phone||null,p_initial_status:status,p_lat:Number(lat),p_lng:Number(lng),p_assigned_rep:canManage()?$('fRep').value:null,p_initial_reason:(status==='hesitant'||status==='rejected')?reason:null});
 if(error)return flash((lang==='ar'?'تعذر إضافة العميل: ':'Could not add customer: ')+(error.message==='status reason required'?(status==='hesitant'?'سبب تردد العميل مطلوب':'سبب رفض العميل مطلوب'):error.message),true);
 closeModal();flash(lang==='ar'?'تمت إضافة العميل':'Customer added');await refreshAll();
}

function openCustomerEditor(id){if(!canManage())return;const c=state.customers.find(x=>Number(x.id)===Number(id));if(!c)return;openModal(lang==='ar'?'تعديل العميل':'Edit Customer',`<div class="form-grid"><div><label>${t('customer')}</label><input id="ecName" value="${esc(c.name)}"></div><div><label>${t('area')}</label><input id="ecArea" value="${esc(c.area||'')}"></div><div><label>${t('phone')}</label><input id="ecPhone" value="${esc(c.phone||'')}"></div><div><label>${t('representative')}</label><select id="ecRep">${repOptions(c.assigned_rep)}</select></div><div class="full"><button class="btn" id="saveCustomerEditBtn" data-id="${c.id}">${t('save')}</button></div></div>`);}
...
function openSaleForm(customerId=null){
 ...
 if(requested&&requested.status!=='active')return flash(lang==='ar'?'لا يمكن تسجيل طلبية لهذا العميل. غيّر حالته إلى نشط عن طريق متابعة العملاء أولاً.':'This customer cannot have a sale recorded. Change the status to Active through follow-up first.',true);
 if(!state.customers.some(c=>c.status==='active'))return flash(lang==='ar'?'لا يوجد عميل نشط متاح لتسجيل طلبية.':'There is no active customer available for a sale.',true);
 openModal(lang==='ar'?'تسجيل سحب / فاتورة':'Record Sale / Withdrawal',`<div class="form-grid"><div class="full"><label>${t('customer')}</label>${customerPickerHtml('s',requested?.id||null,true)}</div><div><label>${t('product')}</label><select id="sProduct">${productOptions()}</select></div><div><label>${t('quantity')}</label><input id="sQty" type="number" min="0.01" step="0.01"></div><div><label>${t('value')}</label><input id="sAmount" type="number" min="0.01" step="0.01"></div><div class="full"><label>${t('reference')}</label><input id="sRef"></div><div class="full"><button class="btn" id="saveSaleBtn">${t('save')}</button></div></div>`);
 setTimeout(()=>bindCustomerPicker('s',null,true),0);
}
async function addSale(){
 ...
 if(!customer||customer.status!=='active')return flash(lang==='ar'?'لا يمكن تسجيل الطلبية حتى تكون حالة العميل نشط. غيّر الحالة عن طريق متابعة العملاء أولاً.':'The customer must be Active before recording a sale. Change the status through follow-up first.',true);
 ...
 if(error)return flash(error.message==='customer must be active before recording a sale'?(lang==='ar'?'يجب أن تكون حالة العميل نشط قبل تسجيل الطلبية.':'Customer must be Active before recording a sale.'):error.message,true);
 closeModal();flash(t('saved'));await refreshAll();
}
...
$('dashboard')?.addEventListener('click',e=>{let el;if((el=e.target.closest('[data-dashboard-link]'))){const k=el.dataset.dashboardLink;if(k==='customers'){state.customerMonthOnly=false;$('customerStatusFilter').value='';$('customerSearch').value='';gotoPage('customers');renderCustomers();}else if(k==='sales-month'){$('salePeriodFilter').value='month';$('saleSearch').value='';gotoPage('sales');renderSales();}else if(['active','hesitant','rejected'].includes(k)){state.customerMonthOnly=false;$('customerStatusFilter').value=k;$('customerSearch').value='';gotoPage('customers');renderCustomers();}}else if((el=e.target.closest('[data-rep-customers]'))){const p=state.profiles.find(x=>x.id===el.dataset.repCustomers);state.customerMonthOnly=false;$('customerStatusFilter').value='';$('customerSearch').value=p?.full_name||'';gotoPage('customers');renderCustomers();}else if((el=e.target.closest('[data-goal-kind]'))){
   ...
   if(kind==='total_sales'){
     ...
     state.customerMonthOnly=true;$('customerSearch').value=rep?.full_name||'';$('customerStatusFilter').value=kind==='active_new_customers'?'active':'';gotoPage('customers');renderCustomers();
   }
 }});
