...
async function renderMap(){
  if(!isAdmin())return;
  if(!state.map){state.map=L.map('map').setView([24.78,46.76],11);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(state.map);state.markerLayer=L.layerGroup().addTo(state.map);}
